...
We revised the results section to reflect the addition of this new figure.
Also, we editted figure captions as appropriate.
...
Specifically, we have carefully edited the captions to provide a clearer understanding of the y-axis by replacing the confusing term "occupancy" with the more explicit phrase "proportion of patch state." We also changed the presentation so the readers can understand what transitions happened (color codes changed).
...
P's trophic position depends on the relative abundance of B and C (see equations 12 & 13).
...
> Thank you for your input. "*Looking at the figure 1 c (and from experience) the expectation is that as omnivory increases the FCL decreases.*" -- After some thought, we realized that the response of FCL to increasing omnivory can be more complex.
...
We have rephrased the corresponding paragraph in the Introduction to reflect this.
...
We clarified the values of ecosystem size or complexity in the main figures (both are randomly drawn from uniform distributions).
...
> Also, we thought those state transtion figures are better placed in the main text so the readers can see the associated text more easily.
We have moved those figures to the main text.
