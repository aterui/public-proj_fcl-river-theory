...
While recent research efforts of food web ecologists have substantially advanced our understanding of food chain drivers, most of the previous work has been conducted in simple, two-dimensional systems, such as lakes [@postEcosystemSizeDetermines2000; @doiResourceAvailabilityEcosystem2009] and oceanic islands [@takimotoEcosystemSizeNot2008; @pringlePredatorinducedCollapseNiche2019]. However, many natural habitats have a high degree of spatial complexity that cannot be represented simply by ecosystem size. Branching ecosystems, for example, are ubiquitous yet overlooked landscape structures (e.g., rivers, trees, caves, mountain ranges) [@grantLivingBranchesPopulation2007; @altermattDiversityRiverineMetacommunities2013]. While we know that food web interactions are spatially influenced [@calcagnoConstraintsFoodChain2011; @pillaiMetacommunityTheoryExplains2011; @takimotoEffectsProductivityDisturbance2012; @teruiSpatialDisturbanceSynchrony2019], the theory to account for it has lagged, particularly in branching habitat architectures. Here, we theoretically explore drivers of FCL in rivers, an excellent example of branching ecosystems.
...
$$
\text{logit}~m_{down} = \text{logit}~m_{up}+ \varepsilon
$$
...
$$
\text{logit}~m_{down} = \omega~(\text{logit}~m_{1,up} +\varepsilon_1)+(1-\omega)~(\text{logit}~m_{2,up} +\varepsilon_2)
$$
...
$$
A_{i,x}(t+1) \sim \text{Poisson}(\bar{A}_{i,x}(t) + I_{i,x}(t) - E_{i,x}(t))
$$

where $\bar{A}_{i,x}(t)$ is the expected species abundance after within-patch trophic interactions, $I_{i,x}(t)$ the expected number of immigrants into patch $x$, and $E_{i,x}(t)$ the expected number of emigrants from patch $x$. The realized discrete number of individuals is drawn from a Poisson distribution to account for stochasticity in demographic and dispersal processes.

***Internal dynamics.*** Each time step starts with internal dynamics. We accounted for background mortality, predator-induced mortality, and disturbance-induced mortality before the density-dependent growth occurs:

$$
\begin{aligned}
A'_{i,x} &= s_{0,i}~\beta_{i,x}(t)~A_{i,x}(t)\\
\bar{A}_{i,x}(t) &= g_i(A)~A'_{i,x}~\gamma_{x}(t)
\end{aligned}
$$

where $s_{0,i}$ is the background survival probability (density independent), $\beta_{i,x}(t)$ the survival probability after predation, and $\gamma_{x}(t)$ the survival probability after stochastic disturbance. The function $g_i(\centerdot)$ defines the form of the density-dependent recruitment for species $i$. Below, we explain how we described predation and recruitment processes for each species.

For the basal species, we considered the predation impacts of consumer and predator species:

$$
\beta_{B,x}(t) = 1 - [z_{BC,x}(t) + z_{BP,x}(t)]
$$
...
$$
g_B(A) = \frac{r_{b}} {1 + \frac{r_b - 1}{K_{x}}A_{B,x}'}
$$

where $r_b$ is the reproductive rate for survived individuals.

For the consumer species, the predation-induced mortality is the proportion of individuals consumed by the predator species:

$$
\beta_{CP,x}(t) = 1 - z_{CP,x} = 1 - \frac{W_{CP,x}}{A_{C,x}(t)}
$$
...
&e_{BC} \left( \frac{W_{BC,x}(t)}{W_{B,x}(t)} A_{B,x}(t) \right) &&\text{if $W_{B,x}(t) > A_{B,x}(t)$}\\\end{aligned}\right.
$$
...
The predator species is not exposed to predation, thus $\beta_{P,x}(t) = 1$. The population growth of the predator considers two prey species:
