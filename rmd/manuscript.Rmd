...
$$
\hat{A}_{P,x}(t) =\left\{\begin{aligned}&s_{0,P} \left[e_{BP} \delta_{x}(t) W_{BP,x}(t) + e_{CP} (1 - \delta_{x}(t)) W_{CP,x}(t) \right] &&\text{if~$W_{B,x}(t) < A_{B,x}(t)$}\\&s_{0,P} \left[e_{BP} \left(\frac{\delta_{x}(t)W_{BP,x}(t)}{W_{B,x}(t)}A_{B,x}\right) + e_{CP} (1 - \delta_{x}(t)) W_{CP,x}(t) \right] &&\text{if~$W_{B,x}(t) > A_{B,x}(t)$}\\\end{aligned}\right.
$$
...
JP developed the theoretical model of trophic dynamics, ran the simulations, and made the figures with input from AT. JP wrote the first draft, while AT and JF contributed significantly to the final version.
