...
We also calculated the proportion of the patch state ("No species", "B", "B + C", "B + P", and "B + C + P") at each time step to infer mechanisms that drive changes in FCL. The time-averaged proportion of patch state $\bar{s}$ was obtained as $\bar{s} = \frac{s_t}{N_p T}$, where $s_t$ is the number of patches in a given patch state at time step $t$.
...
We obtained the global FCL $\bar{\tau}$ and $\bar{s}$ for the last `r unique(df_sim$n_timestep)` time steps. All analysis was performed in R 4.2.1 [@rcoreteamLanguageEnvironmentStatistical2021]
