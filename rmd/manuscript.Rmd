...
No data were used or produced in this research. R functions for simulations are provided as the R package mcbrnet version 1.2.3 (available at <https://github.com/aterui/mcbrnet>). Codes for simulations, statistical analysis, figures, and tables are available at <https://github.com/aterui/public-proj_fcl-river-theory>.
...
***Internal dynamics.*** Each time step starts with internal dynamics. The general formula is as follows:

$$
\begin{aligned}
B_{x,t+1} &= B_{x,t}g(B_{x,t})f_1(B_{x,t}, C_{x,t})f_2(B_{x,t}, C_{x,t})\\
C_{x,t+1} &= e_{BC}B_{x,t}[1 - f_1(B_{x,t}, C_{x,t})]f_3(C_{x,t}, P_{x,t})\\
P_{x,t+1} &= e_{BP}B_{x,t}f_1(B_{x,t}, C_{x,t})[1 - f_2(B_{x,t}, P_{x,t})] + e_{CP}C_{x,t}[1 - f_3(C_{x,t}, P_{x,t})]
\end{aligned}
$$

We modeled density-dependent growth after accounting for background and predator-induced mortality:

$$
\begin{aligned}
\begin{aligned}
A'_{i,x} &= s_{0,i} \beta_{i,x}(t) A_{i,x}(t)\\
\bar{A}_{i,x}(t) &= g_i(A) A'_{i,x}
\end{aligned}
&&\text{(5)}
\end{aligned}
$$

where $s_{0,i}$ is the background survival probability (density independent) and $\beta_{i,x}(t)$ the survival probability after predation. The function $g_i(\cdot)$ defines the form of the density-dependent recruitment for species $i$. Below, we explain how we described predation and recruitment processes for each species.

*Basal species* -- We considered the predation impacts of consumer and predator species:

$$
\begin{aligned}
\beta_{B,x}(t) &= 1 - [z_{BC,x}(t) + z_{BP,x}(t)] &&\text{(6)}
\end{aligned}
$$ where $z_{BC,x}(t)$ and $z_{BP,x}(t)$ are the proportional mortality of basal species caused by consumer and predator species, respectively. The total mortality is calculated as the proportion of total individuals consumed ($W_{B,x}(t)$) relative to the current abundance of the basal species ($A_{B,x}(t)$):
...
$$
\begin{aligned}
W_{B,x}(t) &= W_{BC,x}(t) + \delta_{x}(t) W_{BP,x}(t) &&\text{(8)}
\end{aligned}
$$\
$W_{ij,x}(t)$ is the population-wide consumption of consumer $j$ on prey $i$ (number of individuals), and the parameter $\delta_{x}(t)$ is the relative preference of predator to basal species over consumer. We assume the Holling Type-II functional response for predator-prey interactions:

$$
\begin{aligned}
W_{ij,x}(t) &= \frac{a_{ij}A_{i,x}(t)A_{j,x}(t)}{A_{j,x}(t) + a_{ij}h_{ij}A_{i,x}(t)} &&\text{(9)}
\end{aligned}
$$
...
$$
\begin{aligned}
g_B(A) = \frac{r_{b}} {1 + \frac{r_b - 1}{K_{x}}A_{B,x}'} &&\text{(11)}
\end{aligned}
$$

where $r_b$ is the reproductive rate for survived individuals.

*Consumer species* -- We modeled the predation-induced mortality as the proportion of individuals consumed by the predator species:

$$
\begin{aligned}
\beta_{CP,x}(t) = 1 - z_{CP,x} = 1 - \frac{W_{CP,x}}{A_{C,x}(t)} &&\text{(12)}
\end{aligned}
$$

The population growth of the consumer is controlled by conversion efficiency and the number of prey captured:
...
\left\{
\begin{aligned}
&e_{BC}W_{BC,x}(t) &&\text{if $W_{B,x}(t) < A_{B,x}(t)$}\\
&e_{BC} \left( \frac{W_{BC,x}(t)}{W_{B,x}(t)} A_{B,x}(t) \right) &&\text{if $W_{B,x}(t) > A_{B,x}(t)$}
\end{aligned}
\right.
&&\text{(13)}
\end{aligned}
...
*Predator species* -- The predator species is not exposed to predation, thus $\beta_{P,x}(t) = 1$. The population growth of the predator considers two prey species:
...
\left\{
\begin{aligned}
&e_{BP} \delta_{x}(t) W_{BP,x}(t) + e_{CP} (1 - \delta_{x}(t)) W_{CP,x}(t) &&\text{if $W_{B,x}(t) < A_{B,x}(t)$}\\
&e_{BP} \delta_{x}(t)\left(\frac{W_{BP,x}(t)}{W_{B,x}(t)}A_{B,x}\right) + e_{CP} (1 - \delta_{x}(t)) W_{CP,x}(t) &&\text{if $W_{B,x}(t) > A_{B,x}(t)$}\\
\end{aligned}
\right.
&&\text{(14)}
\end{aligned}
...
***Disturbance.*** After recruitment, all species experience proportional loss of $m$ if stochastic regional disturbance occurs, modifying equation (5) as:

$$
\begin{aligned}
\bar{A}_{i,x}(t) &= g_i(A) A'_{i,x} (1 - m_{x}) &&\text{(15)}
\end{aligned}
$$

where $m_x$ is the disturbance intensity at patch $x$ as determined by the network generation process. Regional disturbance occurs with probability $p_m$.
