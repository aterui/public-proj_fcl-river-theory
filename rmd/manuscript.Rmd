...
***Internal dynamics.*** Each time step starts with internal dynamics. The general formula is given as follows:
...
The parameter $e_{ij}$ is the energetic conversion efficiency from $i$ to $j$. The function $g(\cdot)$ defines the density-dependent recruitment for the basal species, and the function $f_k(\cdot)$ dictates the proportion of survived prey individuals after predation. We used the Beverton-Holt function to describe the density-dependent growth of the basal species:
...
The parameter $a_{ij}$ defines the encounter rate between $j$ and $i$, $h_{ij}$ the handling time, and $\phi$ the switching function for the intraguild predator. We assumed that the degree of omnivory is dependent on the relative abundance of $B''_{x,t}$ and $C'_{x,t}$:

$$
\phi = \frac{s(B''_x(x,t) - C'_{x,t})}{B''_x(x,t) + C'_{x,t}}
$$
...
Prior to the main simulation, we performed a one-patch simulation to explore the predation parameter space ($a_{ij}$ and $h_{ij}$) that allows stable coexistence of the three species in the absence of disturbance. In this simulation, we assumed $h_{ij} = h$ to reduce the dimension of parameter space. This analysis revealed that the stable coexistence of the three species is possible with a certain combinations of predation parameters. Within this parameter space, we explored the three scenarios of food web structure: "chain" ($(a_{BC}, a_{BP}, a_{CP}, h) = (X, X, X, X)$), "weak omnivory" ($(a_{BC}, a_{BP}, a_{CP}, h) = (X, X, X)$), and "strong omnivory" ($(a_{BC}, a_{BP}, a_{CP}, h) = (X, X, X)$). We used $s = 1$ except for the chain scenario with $s = 0$. Without the loss of generality, we assumed $e_{ij} = e = 1$.
