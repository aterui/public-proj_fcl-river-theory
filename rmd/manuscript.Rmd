...
\bar{A}_{i,x}(t) &= g_i(A'_{i,x})~A'_{i,x}~\gamma_{x}(t)
\end{aligned}
$$
...
$$
where $z_{BC,x}(t)$ and $z_{BP,x}(t)$ are the proportional mortality of basal species caused by consumer and predator species, respectively. The total mortality is calculated as the proportion of total individuals consumed ($W_{B,x}(t)=W_{BC,x}(t) + \delta_{x}(t) W_{BP,x}(t)$) relative to the current abundance of the basal species ($A_{B,x}(t)$):

$$
z_{BC,x}(t) + z_{BP,x}(t) =\left\{\begin{aligned}&\frac{W_{BC,x}(t) + \delta_{x}(t) W_{BP,x}(t)}{A_{B,x}(t)} &&\text{if $W_{B,x}(t) < A_{B,x}(t)$}\\&1 &&\text{if $W_{B,x}(t) > A_{B,x}(t)$}\\\end{aligned}\right.
$$
...
$$
g_B(A'_{B,x}) = \frac{r_{b}} {1 + \frac{r_b - 1}{K_{x}}A_{B,x}'}
$$
...
&e_{BC} \left( \frac{W_{BC,x}(t)}{W_{B,x}(t)} A_{B,x} \right) &&\text{if $W_{B,x}(t) > A_{B,x}(t)$}\\\end{aligned}\right.
$$
