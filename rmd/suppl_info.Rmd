...
We performed a sensitivity analysis of the trophic community simulation to identify key simulation parameters that strongly affect the relationships between food chain length and ecosystem properties (the number of habitat patches $N_{p}$ and branching probability $P_{b}$. We generated 500 sets of parameter combinations by randomly drawing values of `r length(attributes(fit_sense$fit[[1]]$terms)$term.label)` simulation parameters from uniform distributions (**Table S1**). For each parameter combination, we generated `r max(sim_sv$n_rep)` branching networks with the gradients of ecosystem size ($N_p \sim \text{Unif}(10, 150)$) and complexity ($P_b \sim \text{Unif}(0.01, 0.99)$). This results in a total of `r nrow(sim_sv)` simulation replicates. We ran `r n_sim_stvy` time steps of trophic community dynamics in each simulation replicate. We obtained temporal means of food chain length for the last `r unique(sim_sv$n_timestep)` time steps. The first `r n_sim_stvy - unique(sim_sv$n_timestep)` time steps were discarded as initialization and burn-in periods.
...
```{=tex}
\begin{align}
  u_{n} &\sim \text{Normal}(\mu_n, \sigma^2)\\
