...
We performed a sensitivity analysis of the trophic community simulation to identify key simulation parameters that strongly affect the relationships between food chain length and ecosystem properties (the number of habitat patches $N_{p}$ and branching probability $P_{b}$. We generated 500 sets of parameter combinations by randomly drawing values of 16 simulation parameters from uniform distributions (**Table S1**). For each parameter combination, we generated `r max(sim_stvy_result$n_rep)` branching networks with the gradients of ecosystem size ($N_p \sim Unif(10, 150)$) and complexity ($P_b \sim Unif(0.01, 0.99)$). This results in a total of `r nrow(sim_stvy_result)` simulation replicates. We ran `r n_sim_stvy` time steps of trophic community dynamics in each simulation replicate. We obtained temporal means of food chain length for the last `r unique(sim_sv$n_timestep)` time steps. The first `r n_sim_stvy - unique(sim_sv$n_timestep)` time steps were discarded as initialization and burn-in periods.
...
\end{align}
```
where $\zeta_k~(k = 0...14)$ are the intercept and regression coefficients, and $x_{k,n}$ are the simulation parameters drawn from uniform distributions (**Table S1**). Explanatory variables were standardized to a mean of zero and a standard deviation of one, so that regression coefficients are comparable.
